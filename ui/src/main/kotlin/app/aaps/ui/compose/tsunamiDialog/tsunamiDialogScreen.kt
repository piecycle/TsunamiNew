...
fun TsunamiDialogScreen(
    ...
    // Dialog states
    // The master's prepared confirmation (actionId + its merged lines), set via the ShowConfirmation side effect.
    ...
    LaunchedEffect(Unit) {
        viewModel.sideEffect.collect { effect ->
            when (effect) {
                ...
                is TsunamiDialogViewModel.SideEffect.ShowConfirmation -> {
                    confirmation = effect.actionId to effect.lines
                }
            }
        }
    }
    ...
    confirmation?.let { (actionId, lines) ->
        ElementConfirmationDialog(
            elementType = ElementType.TSUNAMI,
            lines = lines,
            onConfirm = {
                viewModel.commit(actionId)
                confirmation = null
                onNavigateBack()
            },
    ...
    TsunamiDialogContent(
        ...
        onCancelTsunamiClick = { viewModel.prepareCancelTsunami() }
    )
}
    ...
    Scaffold(
        ...
        bottomBar = {
            Column(
                ...
                // Cancel Tsunami — only shown while Tsunami mode is currently active.
                if (uiState.isTsunamiActive) {
                    OutlinedButton(
                        onClick = {
                            focusManager.clearFocus()
                            onCancelTsunamiClick()
                        },
                        modifier = Modifier.fillMaxWidth()
                    ) {
                        Text(stringResource(CoreUiR.string.cancel_tsunami))
                    }
                }
        ...
        Column(
            ...
            // --- Card 2: Tsunami duration (max 5h) ---
            Card(
                modifier = Modifier.fillMaxWidth(),
                colors = CardDefaults.cardColors(containerColor = MaterialTheme.colorScheme.surfaceContainer)
            ) {
                Column(modifier = Modifier.padding(horizontal = 16.dp, vertical = 8.dp)) {
                    NumberInputRow(
                        labelResId = CoreUiR.string.tsunami_duration,
